...
func NewChatService(chatUC *biz.ChatUsecase, agentUC *biz.AgentUsecase, toolUC *biz.ToolUsecase, skillUC *biz.SkillResourceUsecase, growthUC *biz.GrowthUsecase, channelUC *biz.ChannelUsecase, logger log.Logger) *ChatService {
	return newChatService(chatUC, agentUC, toolUC, skillUC, growthUC, channelUC, memory.NewSessionMemory(), logger)
}

// NewChatServiceWithMemoryStore creates a ChatService with the durable session
// memory backend supplied by the data layer.
func NewChatServiceWithMemoryStore(chatUC *biz.ChatUsecase, agentUC *biz.AgentUsecase, toolUC *biz.ToolUsecase, skillUC *biz.SkillResourceUsecase, growthUC *biz.GrowthUsecase, channelUC *biz.ChannelUsecase, sessionUnits memory.SessionUnitsBackend, logger log.Logger) *ChatService {
	return newChatService(chatUC, agentUC, toolUC, skillUC, growthUC, channelUC, sessionUnits, logger)
}

// ProvideChatServiceWithTurnTrace builds ChatService with durable memory and turn-trace store (wire).
func ProvideChatServiceWithTurnTrace(chatUC *biz.ChatUsecase, agentUC *biz.AgentUsecase, toolUC *biz.ToolUsecase, skillUC *biz.SkillResourceUsecase, growthUC *biz.GrowthUsecase, channelUC *biz.ChannelUsecase, sessionUnits memory.SessionUnitsBackend, turnTraceStore turntrace.Store, d *data.Data, logger log.Logger) *ChatService {
	WireMemoryHubFromData(d)
	s := NewChatServiceWithMemoryStore(chatUC, agentUC, toolUC, skillUC, growthUC, channelUC, sessionUnits, logger)
...
func newChatService(chatUC *biz.ChatUsecase, agentUC *biz.AgentUsecase, toolUC *biz.ToolUsecase, skillUC *biz.SkillResourceUsecase, growthUC *biz.GrowthUsecase, channelUC *biz.ChannelUsecase, sessionUnits memory.SessionUnitsBackend, logger log.Logger) *ChatService {
	...
	chat.InitLocalMemoryHub() // Catalog defaults=local; does not change Prefetch
...
func (s *ChatService) SendMessage(ctx context.Context, req *chatv1.SendMessageRequest) (*chatv1.MessageReply, error) {
	...
	agentMeta, err := s.agentUC.GetForUse(ctx, session.AgentID)
	if err != nil {
	...
	// 加载工具
	tools, err := s.toolUC.ListByAgent(ctx, session.AgentID)
	if err != nil {
	...
	regResult, err := chat.BuildRegistry(tools, reg)
	if err != nil {
	...
	maxHistory := 20
	a := chat.BuildReActAgent(m, reg, agentMeta.SystemPrompt, maxHistory, append(chat.ReActOptionsFromAgent(*agentMeta), s.growthReActOptions(agentMeta.Workspace)...)...)
...
func (s *ChatService) SendMessageStream(ctx context.Context, req *chatv1.SendMessageRequest) (<-chan ChatStreamEvent, string, error) {
	...
	a := chat.BuildReActAgent(m, reg, agentMeta.SystemPrompt, maxHistory,
		append(chat.ReActOptionsFromAgent(*agentMeta),
			append(s.growthReActOptions(agentMeta.Workspace), agent.WithReActEventBus(turnBus))...)...)
	...
	if ir == nil {
		messages = append(messages, model.Message{Role: "user", Content: userContent})
	} else {
