...
func NewHTTPServer(c *conf.Server, tool *service.ToolService, agent *service.AgentService, chat *service.ChatService, channelSvc *service.ChannelService, cronSvc *cron.CronService, channelUC *biz.ChannelUsecase, identityRepo biz.IdentityRepo, aclAPI *biz.ACLAPIUsecase, authUC *biz.AuthUsecase, mcpServer *service.McpServerService, proxy *service.ProxyService, runtimeSvc *runtime.Service, pinger DBPinger, agentUC *biz.AgentUsecase, codeRoots []string, logger log.Logger) *httptransport.Server {
	...
	r.GET("/api/v1/model-providers", ListModelProvidersHandler(chat))
	r.POST("/api/v1/model-providers", CreateModelProviderHandler(chat))
	r.GET("/api/v1/model-providers/{id}", GetModelProviderHandler(chat))
	r.PATCH("/api/v1/model-providers/{id}", PatchModelProviderHandler(chat))
	r.DELETE("/api/v1/model-providers/{id}", DeleteModelProviderHandler(chat))
	r.POST("/api/v1/model-providers/{id}/sync", SyncModelProviderHandler(chat))
	r.GET("/api/v1/model-catalog", ListModelCatalogHandler(chat))
	r.POST("/api/v1/model-catalog", CreateModelCatalogHandler(chat))
	r.PATCH("/api/v1/model-catalog/{id}", PatchModelCatalogHandler(chat))
	r.DELETE("/api/v1/model-catalog/{id}", DeleteModelCatalogHandler(chat))
	r.GET("/api/v1/agents/{agent_id}/model-choices", ListModelChoicesHandler(chat))
	r.PATCH("/api/v1/sessions/{session_id}/model", PatchSessionModelHandler(chat))
