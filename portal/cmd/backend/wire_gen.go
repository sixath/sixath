...
func wireApp(confServer *conf.Server, confData *conf.Data, auth *conf.Auth, growth *conf.Growth, logger log.Logger) (*kratos.App, func(), error) {
	...
	toolUsecase := biz.NewToolUsecase(toolRepo, resourceRepo, accessChecker, proxyRepo, logger)
	...
	agentUsecase := biz.NewAgentUsecase(agentRepo, resourceRepo, accessChecker, proxyRepo, string2, logger)
	...
	chatService := service.ProvideChatServiceWithTurnTrace(chatUsecase, agentUsecase, toolUsecase, mcpServerUsecase, skillResourceUsecase, channelUsecase, sessionUnitsBackend, store, v, dataData, proxyRepo, logger)
	...
	proxyUsecase := biz.NewProxyUsecase(proxyRepo, resourceRepo, accessChecker, logger)
	proxyService := service.NewProxyService(proxyUsecase, logger)
